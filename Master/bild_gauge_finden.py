...
url='rotated_image1.jpg'
image_path = 'weronek_gauge2_1.jpg'
...
def take_measure(threshold_img, threshold_ln, minLineLength, maxLineGap, diff1LowerBound, diff1UpperBound, diff2LowerBound, diff2UpperBound):
    ...
    if circles is not None:
        ...
        cv2.circle(img, (x, y), 10, (0, 255, 0), 3, cv2.LINE_AA)  # draw center of circle
        ...
        for i in range(0,interval):
            ...
            cv2.putText(img, '%s' %(int(i*separation)), (int(p_text[i][0]), int(p_text[i][1])), cv2.FONT_HERSHEY_SIMPLEX, 0.9,(255,0,0),1,cv2.LINE_AA)
